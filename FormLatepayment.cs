...
{
    public partial class FormLatepayment : UserControl
    {
        public FormLatepayment()
        {
            InitializeComponent();
        ...
        private void Updatebtn_Click(object? sender, EventArgs e)
        {
            try
            {
                using (SqlCommand cmd = new SqlCommand("sp_UpdateLatePaymentDetailByLatePaymentNoAndInvoiceNo", Program.Connection))
                {
                    cmd.CommandType = CommandType.StoredProcedure;

                    // Replace control names with yours
                    cmd.Parameters.AddWithValue("@LatePaymentNo", Convert.ToInt32(txtno.Text));
                    ...
                    MessageBox.Show("LatePayment detail updated successfully.");
                }
            }
            catch (Exception ex)
            {
        ...
        private void Tbnupdate_Click(object? sender, EventArgs e)
        {
            try
            {
                using (SqlCommand cmd = new SqlCommand("sp_UpdateLatePaymentByLatePaymentNo", Program.Connection))
                {
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.AddWithValue("@LatePaymentNo", Convert.ToInt32(txtno.Text));
                    cmd.Parameters.AddWithValue("@LatePaymentDate", dppay.Value);
                    ...
                    MessageBox.Show("LatePayment updated successfully.");
                }
            }
            catch (Exception ex)
            {
        ...
        private void DataGridView1_Click(object? sender, EventArgs e)
        {
            try
            {
                ...
                {
                    int LatePaymentNo = Convert.ToInt32(dataGridView1.CurrentRow.Cells["LatePaymentNo"].Value);
                    int invoiceNo = Convert.ToInt32(dataGridView1.CurrentRow.Cells["InvoiceNo"].Value);
                    cbid.Text = Convert.ToString(LatePaymentNo);
                    using (SqlCommand cmd = new SqlCommand("spGetLatePayment", Program.Connection))
                    {
                        cmd.CommandType = CommandType.StoredProcedure;
                        cmd.Parameters.AddWithValue("@PaymentNo", LatePaymentNo);
                        ...
                        using (SqlDataReader reader = cmd.ExecuteReader())
                        {
                            if (reader.Read())
                            {
                                // Fill controls with LatePayment info
                                txtno.Text = reader["LatePaymentNo"].ToString();
                                dppay.Value = Convert.ToDateTime(reader["LatePaymentDate"]);
                            ...
                            {
                                MessageBox.Show("No LatePayment record found for this row.");
                            }
                        }
        ...
        private void Textsearch_TextChanged(object? sender, EventArgs e)
        {
            if (textsearch.Text != "")
            {
                try
                {
                    int LatePaymentNo = Convert.ToInt32(textsearch.Text);

                    using (SqlCommand cmd = new SqlCommand("spGetLatePaymentByLatePaymentNoAndInvoiceNo", Program.Connection))
                    {
                        cmd.CommandType = CommandType.StoredProcedure;

                        // For demo: same value used as LatePaymentNo and InvoiceNo
                        // You can separate these from different inputs if needed
                        cmd.Parameters.AddWithValue("@LatePaymentNo", LatePaymentNo);
                        ...
                        DataTable filteredTable = dt.DefaultView.ToTable(false,
                            "LatePaymentNo", "InvoiceNo", "TenantID", "PaidAmount", "LatePaymentDate");
        ...
        private void Btninsert_Click(object? sender, EventArgs e)
        {
            try
            {
                using (SqlCommand cmd = new SqlCommand("sppInsertLatePaymentDetail", Program.Connection))
                {
                    cmd.CommandType = CommandType.StoredProcedure;
                    ...
                    MessageBox.Show("LatePayment detail inserted successfully.");
                }
            }
            catch (Exception ex)
            {
                MessageBox.Show("Error inserting LatePayment detail: " + ex.Message);
                if (Program.Connection.State == ConnectionState.Open)
                    Program.Connection.Close();
            }
        }
        ...
        private void Btnsave_Click(object? sender, EventArgs e)
        {
            try
            {
                using (SqlCommand cmd = new SqlCommand("spInsertLatePayment", Program.Connection))
                {
                    cmd.CommandType = CommandType.StoredProcedure;
                    ...
                    MessageBox.Show("LatePayment inserted successfully.");
                }
            }
            catch (Exception ex)
            {
                MessageBox.Show("Error inserting LatePayment: " + ex.Message);
                if (Program.Connection.State == ConnectionState.Open)
                    Program.Connection.Close();
            }
        ...
        private void Btnnew_Click(object? sender, EventArgs e)
        {
            ...
            SqlCommand cmd = new SqlCommand("sp_CountLatePayments", Program.Connection);
            cmd.CommandType = CommandType.StoredProcedure;
            ...
            using (SqlDataReader reader = cmd.ExecuteReader())
            {
                if (reader.Read())
                {
                    totalLatePayments = Convert.ToInt32(reader["TotalLatePayments"]) + 1;
                }
            }
            Program.Connection.Close();

            // Display in a textbox
            txtno.Text = totalLatePayments.ToString();
        ...
        private void Cbid_SelectedIndexChanged(object? sender, EventArgs e)
        {
            ...
            if (cbid.SelectedValue != null && cbid.SelectedValue.ToString() != "")
                try
                {
                    ...
                    {
                        MessageBox.Show("This Invoice is Late , Please Pay vai LateLatePaymentForm ", "Late LatePayment", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                        Program.Connection.Close();
                    }
                    Program.Connection.Close();
